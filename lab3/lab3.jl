...
# ╔═╡ ab4b1f06-aedd-11ec-16f0-1199971601ab
begin
	using CSV
	using DataFrames
	using StatsPlots
...
# ╔═╡ 786d31d0-5aaf-4a09-a578-2443cd161c1b
df = CSV.read("lab3.csv", DataFrame)

# ╔═╡ f4c4209e-97c1-4e10-91eb-a266430709a9
fitparams = let
	x = 1000 ./ df[!,3]
	y = 1 .- cosd.(df[!,1])
	x̄ = mean(x)
...
# ╔═╡ f4e56be2-8d95-46a2-83c7-c87ec167a85c
let
	m = fitparams[:m]
	b = fitparams[:b]
	x = 1000 ./ df[!,3]
	...
	plot!(x -> m*x + b, label="Fit y = $(round(m, digits=2)) x + ($(round(b, digits=2)))", title="Compton Effect experiment linear fit", xlabel="1/E (MeV⁻¹)", ylabel="1 - cos(θ)", color=:black)
	plot!()
end

# ╔═╡ 11e3a02e-6333-4a87-b631-0712947bd262
500.233u"keV" |> u"MeV"

# ╔═╡ 3aa50088-5fbd-42c4-8b7d-25bf047268b4
df[!,4] / 1000
...
manifest_format = "2.0"
project_hash = "7ef1afe3d02faaf3b0967bb8d715047fd160e51d"
